...
public:
    ...
    int minimumSum(vector<vector<int>> &grid)
    {
        ...
        // Case 3 & 4: Vertical cut, then one side split horizontally
        for (int c = 0; c < n - 1; ++c)
        {
            for (int r = 0; r < m - 1; ++r)
            {
                // Case 3a: Left is one rect, right is split horizontally
                int area1 = getarea(grid, 0, m - 1, 0, c);
                int area2 = getarea(grid, 0, r, c + 1, n - 1);
                int area3 = getarea(grid, r + 1, m - 1, c + 1, n - 1);
                ans = min(ans, area1 + area2 + area3);

                // Case 3b (FIX): Right is one rect, left is split horizontally
        ...
        // Case 5: Three horizontal stripes
        ...
        for (int c1 = 0; c1 < n - 2; ++c1)
        {
            for (int c2 = c1 + 1; c2 < n - 1; ++c2)
            {
                ...
                // (FIX) The third region starts at c2 + 1
